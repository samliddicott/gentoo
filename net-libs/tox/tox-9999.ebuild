...
src_configure() {
	econf \
		$(usex log-trace "--enable-log --with-log-level=TRACE" "") \
		$(usex log-debug "--enable-log --with-log-level=DEBUG" "") \
		$(usex log-info "--enable-log --with-log-level=INFO" "") \
		$(usex log-warn "--enable-log --with-log-level=WARNING" "") \
		$(usex log-error "--enable-log --with-log-level=ERROR" "") \
		$(use_enable av) \
